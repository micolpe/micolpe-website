...
function valueOrFallback(value, fallback = tr('Non renseigné', 'Not provided')) {
...
function formatSubscription(profile) {
  const rawType = String(profile?.subscription_type || '').trim().toLowerCase();
...
async function loadPortalData(session, profile) {
  ...
  const [loftResult, devicesResult, pigeonsResult, pedigreeResult] = await Promise.all([
    supabase
      .from('loft')
      .select(
...
function renderMember(profile) {
  const name = valueOrFallback(profile.name, tr('Membre Micolpe', 'Micolpe member'));
...
function renderSummary() {
  ...
  setText('summary-loft', loft ? tr('Configuré', 'Configured') : tr('À compléter', 'To complete'));
  setText(
    'summary-loft-detail',
    loft ? valueOrFallback(loft.nameloft, 'Micolpe loft') : tr('Aucun loft trouvé', 'No loft found'),
  );
  ...
  setText(
    ...
    revokedDevices
      ? tr(`${revokedDevices} appareil${revokedDevices > 1 ? 's' : ''} révoqué${revokedDevices > 1 ? 's' : ''}`, `${revokedDevices} revoked device${revokedDevices > 1 ? 's' : ''}`)
  ...
  setText(
    ...
    pigeonCount > pigeons.length
      ? tr(`${pigeons.length} derniers chargés`, `${pigeons.length} latest loaded`)
...
function renderProfile() {
  ...
  setText('profile-subscription-start', formatDate(profile.subscription_start, true));
  setText('profile-subscription-end', formatDate(profile.subscription_end, true));
...
function buildPedigreeGenerations(root, level) {
  ...
  for (let generation = 1; generation < level; generation += 1) {
    ...
    for (const pigeon of generations[generation - 1]) {
      current.push(pigeonById(pigeon?.father_id), pigeonById(pigeon?.mother_id));
    }
...
function renderPedigreeNode(pigeon, generationIndex, nodeIndex, settings) {
  ...
  node.style.setProperty('--pedigree-node-radius', `${Number(settings.radius) || 0}px`);
  node.style.setProperty('--pedigree-line', pedigreeValue(settings.color_line, '#000000'));
  node.style.setProperty('--pedigree-ring', pedigreeValue(settings.color_ring, '#c91e2e'));
  node.style.setProperty('--pedigree-name', pedigreeValue(settings.color_name, '#111111'));
  node.style.setProperty('--pedigree-fancier', pedigreeValue(settings.color_fancier, '#174ca0'));
  node.style.setProperty('--pedigree-color', pedigreeValue(settings.color_color, '#8b4513'));
  node.style.setProperty('--pedigree-fill', pedigreeValue(membership.frame_color, '#ffffff'));
  if (pedigreeEnabled(settings.with_shadow)) node.classList.add('with-shadow');
  if (pedigreeEnabled(settings.with_separator)) node.classList.add('with-separator');

  const rawGender = String(pigeon?.gender || '').trim().toUpperCase();
...
async function openPedigreePreview(pigeon) {
  ...
  try {
    document.querySelector('#pedigree-modal-title').textContent =
      tr(`Pedigree — ${formatRing(pigeon)}`, `Pedigree — ${formatRing(pigeon)}`);
    document.querySelector('#pedigree-root-ring').textContent = formatRing(pigeon);
    document.querySelector('#pedigree-sheet-heading').textContent = 'Pigeon Pedigree';
    ...
    const loftDetails = isOwner
      ? [
        ...
        pedigreeEnabled(settings.with_address) ? portalState.loft?.addressloft : '',
        pedigreeEnabled(settings.with_website) ? portalState.loft?.website : '',
    ...
    sheet.style.setProperty('--pedigree-font', pedigreeValue(settings.font, 'Roboto'));
    sheet.style.setProperty('--pedigree-title', pedigreeValue(settings.color_title, '#606060'));
    sheet.dataset.level = String(level);
    generationsElement.replaceChildren();

    buildPedigreeGenerations(pigeon, level).forEach((generation, generationIndex) => {
      ...
      generation.forEach((ancestor, nodeIndex) => {
        const node = renderPedigreeNode(ancestor, generationIndex, nodeIndex, settings);
    ...
    footerDate.textContent = pedigreeEnabled(settings.with_date)
      ? new Intl.DateTimeFormat(isEnglish ? 'en-GB' : 'fr-FR').format(new Date())
      : '';
...
function renderDevices() {
  ...
  for (const device of portalState.devices) {
    ...
    row.append(
      ...
      createTextCell(
        device.device_type === 'mobile' ? 'Mobile' : 'Desktop',
      ),
...
function filteredPigeons() {
  ...
  return portalState.pigeons.filter((pigeon) => {
    ...
    const matchesFather =
      !fatherSearch ||
      normalizeSearch(formatRing(pigeonById(pigeon.father_id))).includes(fatherSearch);
    const matchesMother =
      !motherSearch ||
      normalizeSearch(formatRing(pigeonById(pigeon.mother_id))).includes(motherSearch);
    const matchesState = !state || membership.state === state;
    const matchesGender =
      !gender || genderSymbol(pigeon.gender) === gender;
    return (
      matchesRing &&
      matchesName &&
      matchesFather &&
  ...
  if (!visiblePigeons.length) {
    ...
    for (const pigeon of visiblePigeons) {
      ...
      row.append(
        createTextCell(
          formatRing(pigeon),
          valueOrFallback(pigeon.country, ''),
        ),
      ...
      previewButton.addEventListener('click', () => openPedigreePreview(pigeon));
...
function showSection(sectionName) {
  const allowed = ['overview', 'profile', 'loft', 'devices', 'pigeons'];
  const target = allowed.includes(sectionName) ? sectionName : 'overview';
...
function bindInteractions() {
  ...
  document.querySelector('#logout-button').addEventListener('click', async () => {
    ...
    try {
      ...
      showPortalError(tr('La déconnexion a échoué. Réessayez.', 'Sign-out failed. Please try again.'));
    }
  });
  ...
  document.querySelector('#pedigree-export-pdf').addEventListener('click', () => {
    window.print();
  });
  ...
  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && !document.querySelector('#pedigree-modal').hidden) {
...
async function initializePortal() {
  ...
  try {
    ...
    if (!authState) return;
    const data = await loadPortalData(authState.session, authState.profile);
    renderPortal(data);
  } catch {
    showPortalError(
      tr(
